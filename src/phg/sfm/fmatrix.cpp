...
//            std::cout << "(" << x0 << ", " << y0 << "), (" << x1 << ", " << y1 << ")" << std::endl;
            ...
            auto mat1 = cv::Matx31d(x1, y1, 1);
            auto mat0 = cv::Matx13d(x0, y0, 1);
            auto m = (mat1 * mat0).reshape<1, 9>();
...
//             Поправить F так, чтобы соблюдалось свойство фундаментальной матрицы (последнее сингулярное значение = 0)
        ...
        Eigen::VectorXd sigmas = svdf.singularValues();
        sigmas(2) = 0;
        F = svdf.matrixU() * sigmas.asDiagonal() * svdf.matrixV().transpose();
    ...
    cv::Matx33d getNormalizeTransform(const std::vector<cv::Vec2d> &m)
    {
        ...
        double std = sqrt(stddev(0) * stddev(0) + stddev(1) * stddev(1)) / sqrt(2);

        auto T = cv::Matx33d::zeros();
        T(2, 2) = 1;
        T(0, 0) = T(1, 1) = 1 / std;
        T(0, 2) = -mean(0) / std;
        T(1, 2) = -mean(1) / std;
