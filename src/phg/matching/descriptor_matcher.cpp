...
const float THRESHOLD_RATIO_TEST = 0.6;
                                                    ...
                                                    std::vector<cv::DMatch> &filtered_matches)
{
    ...
    for (const auto &match : matches) {
        ...
        if (match.size() > 1 && match[0].distance > THRESHOLD_RATIO_TEST * match[1].distance) {
            continue;
        }
                                                   ...
                                                   std::vector<cv::DMatch> &filtered_matches)
{
    ...
    filtered_matches.clear();
    for (int i = 0; i < n_matches; i++) {
        std::unordered_set<int> neighbors_query;
        for (int j = 0; j < total_neighbours; j++) {
            if (distances2_query.at<float>(i, j) > radius2_query) {
                break;
            }
            neighbors_query.insert(indices_query.at<int>(i, j));
        }
        ...
        for (int j = 0; j < total_neighbours; j++) {
            ...
            if (neighbors_query.find(indices_train.at<int>(i, j)) != neighbors_query.end()) {
                common++;
            }
        }
