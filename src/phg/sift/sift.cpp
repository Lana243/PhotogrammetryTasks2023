...
#define SMOOTH_VALUE 0.25  // коэфициент сглаживания
...
void phg::SIFT::buildPyramids(const cv::Mat &imgOrg, std::vector<cv::Mat> &gaussianPyramid, std::vector<cv::Mat> &DoGPyramid) {
    ...
    for (size_t octave = 0; octave < NOCTAVES; ++octave) {
        if (octave == 0) {
            ...
            cv::resize(img, img, cv::Size(img.cols / 2.0, img.rows / 2.0), 0.5, 0.5, cv::INTER_NEAREST);
        ...
        for (ptrdiff_t layer = 1; layer < OCTAVE_GAUSSIAN_IMAGES; ++layer) {
            ...
            double sigma = INITIAL_IMG_SIGMA * pow(2.0, octave) * sqrt(pow(k, 2 * layer) - 1);
...
namespace {
    std::pair<float, float> parabolaFitting(float x0, float x1, float x2) {
        ...
        float val = a * (shift + 1) * (shift + 1) + b * (shift + 1) + c;
        return std::make_pair(shift, val);
    }
}
                ...
                for (ptrdiff_t j = 1; j < cur.rows - 1; ++j) {
                    for (ptrdiff_t i = 1; i + 1 < cur.cols; ++i) {
                        ...
                        for (int dx = -1; dx <= 1 && (is_min || is_max); ++dx) {
                            if (dx != 0 || dy != 0 || dz != 0) {
                                float val = DoGs[1 + dz].at<float>(j + dy, i + dx);
                                if (center < val) {
                                    is_max = false;
                                }
                                if (center > val) {
                        ...
                        {
                            auto extX = parabolaFitting(DoGs[1].at<float>(j, i - 1), center, DoGs[1].at<float>(j, i + 1));
                            auto extY = parabolaFitting(DoGs[1].at<float>(j - 1, i), center, DoGs[1].at<float>(j + 1, i));
                            dx = extX.first;
                            dy = extY.first;
                            dvalue = (extX.second + extY.second) / 2;
                        }
#endif
                        ...
                        for (size_t bin = 0; bin < ORIENTATION_NHISTS; ++bin) {
                            ...
                            if (value > prevValue && value > nextValue && votes[bin] > biggestVote * ORIENTATION_VOTES_PEAK_RATIO) {
                                auto bin_fixed = parabolaFitting(prevValue, value, nextValue);
                                kp.angle = (bin_fixed.first + 0.5) * (360.0 / ORIENTATION_NHISTS);
...
bool phg::SIFT::buildLocalOrientationHists(const cv::Mat &img, size_t i, size_t j, size_t radius,
    ...
    for (size_t y = j - radius + 1; y < j + radius; ++y) {
        for (size_t x = i - radius + 1; x < i + radius; ++x) {
            ...
            sum[bin] += magnitude;
            sum[(bin + DESCRIPTOR_NBINS - 1) % DESCRIPTOR_NBINS] += SMOOTH_VALUE * magnitude;
            sum[(bin + 1) % DESCRIPTOR_NBINS] += SMOOTH_VALUE * magnitude;
        }
    }
